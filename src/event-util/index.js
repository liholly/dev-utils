export function addHandler(element, type, handler) { //添加事件
...
export function removeHandler(element, type, handler) {  //取消事件
...
export function getEvent(event) {  //使用这个方法跨浏览器取得event对象
	return event ? event : window.event;
}
export function getTarget(event) {  //返回事件的实际目标
	return event.target || event.srcElement;
}
export function preventDefault(event) {   //阻止事件的默认行为
